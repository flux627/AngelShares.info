	...
	function verifyRawTX($hash, $block) {
		if ($hash == "730f0c8ddc5a592d5512566890e2a73e45feaa6748b24b849d1c29a7ab2b2300") {
         ...
         if (isset($decoded_tx['tx-comment'])) $txc_fix = $decoded_tx['tx-comment'];
			...
			$stmt = $this->dbh->prepare('insert into tx (hash, block, message, outputs, inputs, size, version, coinbase) values (?, ?, ?, ?, ?, ?, ?, ?)');
         ...
         */
			...
			if (!$db_id) {$this->decho("FATAL ERROR: database returned no index for tx - txid = $hash"); exit();}
