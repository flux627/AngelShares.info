...
foreach ($txids as $txid) {
	...
	for ($i = 0; $i < count($vin_from_db); $i++) {
		...
		$q = $f->dbh->query("select id from tx where hash = '$vin_txid' and inactive is null");
		$r = $q->fetchAll();
		$prev_tx_db_id = $r[0][0];

		// find the address each vin is "from"
		$q = $f->dbh->query($b = 'select address from vout_address as A join (select id from vout where inactive is null and n = ' . (int)$vin['vout'] . ' and tx_db_id = ' . $prev_tx_db_id . ') as B where A.inactive is null and A.vout_id = B.id');
		$r = $q->fetchAll();
		$address = $r[0][0];
		...
		$q = $f->dbh->query("select id, time from block as A join (select block from tx where id = $tx_db_id and inactive is null) as B where A.id = B.block and A.inactive is null");
		$r = $q->fetchAll();
